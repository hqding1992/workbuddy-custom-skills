...
- **开奖数据 → Step 2 独立处理**
- **活动 / 渠道 / 营销类信息（0.1~0.3, 0.5~0.6, 0.7~0.8）** → **官网为主力信源**，微信公众号（Step 3）为补充。公众号仅用于填补官网覆盖盲区。
...
#### 第4批：华中+西南+西北+东北+中彩网（7个 web_fetch）
...
| 西藏 | 体彩+福彩 | 中彩网聚焦 + Step 3.4 搜索引擎 |
| 宁夏 | 福彩 | 中彩网聚焦 + Step 3.4 搜索引擎 |
| 深圳 | 体彩 | 中彩网聚焦 + Step 3.4 搜索引擎 |
...
| **中彩网补充** | 中彩网"聚焦"栏目在第4批并行抓取，覆盖缺URL省份的活动/渠道信息 |
| **去重** | 与 Step 0（江苏）和 Step 3.1（南京/江苏公众号）结果去重 |
...
> ⚠️ **重要限制**：搜狗微信搜索索引更新严重滞后（2026年实测返回多为2022-2023年旧文章），时效性较差。
> **公众号搜索已从主力信源降级为补充信源**，仅保留南京/江苏本地4条必搜。
> **执行规则**：4条必搜**每次必须执行一次**（轻量搜索，-n 3 即可）。返回结果中超过7天的旧内容自动过滤，保留7天内的有效内容。如搜狗脚本被反爬拦截，降级为 Step 3.4 搜索引擎方案，不得跳过。
...
> ⚠️ **时效性检查**：执行后检查返回文章的发布日期，如果最文章超过 7 天，说明搜狗索引未更新，直接跳过后续公众号搜索。
...
对于 Step 3.1 中 `-r` 解析成功且**发布时间在7天内**的文章，使用 `web_fetch` 抓取全文：
...
| 要点 | 说明 |
|------|------|
| **精简搜索** | 仅保留南京/江苏 4 条必搜，取消全国 30 省 60 条搜索 |
| **时效检查** | 如果 4 条必搜返回结果均超过 7 天，直接跳过，用 3.4 搜索引擎替代 |
| **失败处理** | 如果搜狗脚本被反爬拦截，跳过，降级为 3.4 搜索引擎方案 |
| **用户手动链接** | 如果用户手动提供 `mp.weixin.qq.com/s/xxxxx` 链接，`web_fetch` 可直接抓取全文（成功率接近 100%） |
...
| 规范 | 说明 |
|------|------|
| **基础请求量** | Step 0（11抓取：第1批8并行+第2批2串行）+ Step 1（3搜索）+ Step 2（5抓取：福彩1串行+体彩4并行）+ **Step 2.5（44 web_fetch 分6批（含中彩网）+ 1 API + 9 Playwright串行 =约54次）** + Step 3 公众号（4必搜）+ Step 3.4 搜索引擎（1条）= **约78-88 次/天** |
