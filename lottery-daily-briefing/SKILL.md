...
按 `format-template.md` 的格式生成简报文件（**散文体**：条目用 `N. **标题**` 编号+加粗，内容写通顺叙述，来源/URL 单独成行；**严禁 `### 条目标题` 与 `- **子字段**`**，否则 `_audit.js` 会把它们当独立条目误判，导致 issues/dups 爆表、分发卡死。该约束于 2026-08-05 与审计脚本对齐固化）。
...
*最后更新：2026-08-05（v2.1.19：散文体模板与 _audit.js 对齐 + 成品去噪 + 通用转换器保留中奖/开奖全字段；详见版本发布记录表）*
