...
按 `format-template.md` 的格式生成简报文件（**新闻体**：政策/活动/渠道/中奖每条用 `### 条目标题（日期｜地区）` 三级标题起头，下方写完整叙述段落，来源融入段尾 `（来源：XX（日期））`；开奖/预告用表格。**严禁把字段拆成 `- **子字段**` 独立列表项**（如 `- **案例说明**：`、`- **创新点**：`）——`_audit.js` 会把每个子字段当独立条目、判空正文误报（issues/dups 爆表），简报被卡死。**一条新闻 = 一个 `###` 标题 + 一整段通顺叙述**，不堆 bullet、不堆子字段。该规范于 2026-08-05 对齐 07-30 标杆版式与 `_audit.js` 固化）。

#### ⚠️ 强清洗与垃圾拒收规则（2026-07-29 加固，防 07-28「网页垃圾/历史归档冒充新政」事故复发）

> 07-28 事故根因：① 详情抓取对结构未覆盖的站点（山西 sxlottery.net、海南 hainantc.com.cn）退回 `document.body` 抓前 2000 字 = 导航/CSS 垃圾（已通过 `deepdive.js` 加固：抓到垃圾自动原生 fetch 兜底重抓真正文）；② 生成环节无任何过滤，垃圾/历史归档政策直接进简报。以下规则为生成时的**强制前置过滤**，违反即视为生成不合格。

生成前，对每一条候选内容执行：

1. **正文垃圾拒收**：若某条详情正文符合以下任一，直接丢弃，绝不进简报：
   - 含 CSS 标记（`:hover`、`.class` 选择器链、`{width:...}` 样式块）
   - 含导航/面包屑残留（`网站首页`、`客服热线`、`您所在的位置`、`首页 >>`）
   - 中文占比 < 15%（基本是英文/符号/乱码，非中文正文）
2. **历史归档政策剔除**：标题含 `条例/办法/细则/暂行规定/财务管理办法/管理/印发` 且正文含旧年份 `2003/2008/2009/2012` 等但**不含 2026**，判定为历史归档政策，删除不纳入（例：财综(2003)78号、2009《彩票管理条例》、财综〔2012〕15号）。URL 含 `/07-00000xxx` 等归档栏目路径的整条剔除。
3. **标题净化**：去掉网站分类后缀/前缀——`_行业_中彩网`、`| 中彩网`、`| 体彩/福彩/彩票`、尾部 `(YYYY-MM-DD)` 或 `YYYY年M月D日`。
4. **正文清洗**：剥掉面包屑导航、站点菜单、页脚版权（©、`京ICP`）、"相关新闻"块、JS 残留（`var appendix`、`document.getElementById`）、社交页脚（"点赞 发长微博"、`window.`）。
...
7. **成品去噪（2026-08-05 新增）**：简报成品只含**读者内容**。生成/转换完成后，剔除所有内部注记——① 政策剔除说明（形如 `（注：…按强清洗规则予以剔除…）` 的整段）；② 内部运营标签（`（江苏本地重点关注）`/`(本地重点关注)`/`(特别关注)` 等）；③ 数据校正括号（`(…补发，原题…)`/`(综合转述…)`/`(原文…)` 等，确属来源说明则简化为"来源：XX（日期）"）。上述注记仅留构建日志，不得进入读者简报。此规则是 07-28 强清洗的延伸——此前只管网页垃圾正文，未约束生成阶段漏注。
...
*最后更新：2026-08-05（v2.1.20：模板改回新闻体 `### 条目标题（日期｜地区）` + 完整叙述段落 + 段尾来源 + 充足间隔（对齐 07-30 标杆版式）；继续严禁 `- **子字段**` 拆 bullet；通用转换器同步输出新闻体 + 开奖表格）*
...
| **v2.1.19** | 2026-08-05 | ① 根因修复：模板 `format-template.md` 原强制 `### 条目标题`+`- **子字段**` 与 `_audit.js` 条目解析打架（子字段被当独立条目→空正文/跨节重复误报 200/82，分发卡死）；改为散文体（`N. **标题**`+叙述+来源行），与审计对齐。② 新增成品去噪规则（剔除政策剔除说明/运营标签/数据校正括号）。③ 通用转换器 `scripts/_convert.js`：子字段→散文体还原+去噪，并保留中奖/开奖全部结构化字段（修复初版中奖核心字段丢失）。④ 主任务 13:00 与兜底 14:10 prompt 同步散文体铁律+自愈。 |
| **v2.1.20** | 2026-08-05 | ① 模板改回**新闻体**（对齐 07-30 标杆版式）：政策/活动/渠道/中奖每条用 `### 条目标题（日期｜地区）` 三级标题 + 完整叙述段落 + 段尾 `（来源：XX（日期））`，条目间充足空行、疏朗有新闻感；开奖/预告用表格。② **继续严禁 `- **子字段**` 拆 bullet**（审计误判根因），但 `###` 标题本身审计兼容（每条=单条目+整段正文）。③ 通用转换器 `scripts/_convert.js` 同步输出新闻体（子字段→`###` 条目+叙述；开奖→9 彩种汇总表格），保留全部字段+去噪。④ 主任务 13:00 与兜底 14:10 prompt 同步新闻体规范。 |
